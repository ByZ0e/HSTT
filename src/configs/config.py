...
class SharedConfigs(object):
    ...
    def __init__(self, desc="shared config for pretraining and finetuning"):
        ...
        parser.add_argument(
            "--inference_metric", default=0, type=int, choices=[0, 1],
            help="perform inference run. 0: disable, 1 enable")
        parser.add_argument("--inference_vid_db", type=str,
    ...
    def get_video_qa_args(self):
        ...
        self.parser.add_argument("--ans2label_path", type=str, default=None,
                                 help="path to {answer: label} file")
        self.parser.add_argument(
            "--all_metric", type=int, choices=[0, 1], default=0,
            help="evaluate all metric, 0: disable, 1 enable")
        self.parser.add_argument(
            "--sampled", type=int, choices=[0, 1], default=0,
            help="evaluate all metric, 0: disable, 1 enable")
        ...
        if args.task in ["action", "transition"]:
            ...
            args.num_labels = max(num_answers, 1852)
            args.loss_type = "ce"
        else:
